...
while command != 'exit'
  ...
  when command == 'train'
    ...
    fail 'Error: Bad kind' if train_kind != 'cargo' && train_kind != 'passenger'
    ...
    begin
      if train_kind == 'cargo'
        ...
        train_name = CargoTrain.new(
          name: train_name, factory: train_factory, number: train_number)
      elsif train_kind == 'passenger'
        puts 'Name for passenger Train? '
        train_name = gets.chomp
        train_name = PassengerTrain.new(
          name: train_name, factory: train_factory, number: train_number)
      end
    rescue RuntimeError => e
      puts e.inspect
  ...
  when command == 'carriage_add'
    ...
    begin
      ...
      train_obj.carriage_add(train_obj.kind, carriage_prod, carriage_space)
    rescue RuntimeError => e
      puts e.inspect
    end
  when command == 'carriage_del' # Delete last carriage.
    puts 'Pls tell a name of created train: '
    train_name = gets.chomp
    Train.trains.each { |_n, x| x.carriage_del if x.name == train_name }
  ...
  when command == 'stations_trains'
    RailwayStation.all.each do |_k, i|
      puts "On #{i} station we have: "
      i.trains.each do |trn|
        puts "Train with number: #{trn.number},
         kind - #{trn.kind} and #{trn.carriages} carriages."
        puts "Carriages in that train: #{trn.carriages}"
        trn.carriages.each do |carr|
          puts "Carriage number #{carr.number},
           kind #{carr.kind},
            free space #{carr.free_volume}" if carr.kind == 'cargo'
          puts "Carriage number #{carr.number},
           kind #{carr.kind},
            free size #{carr.free_space}" if carr.kind == 'passenger'
        end
      end
    end
  ...
  when command == 'carriage_list'
    ...
    Train.trains["#{train_number}"].carriages.each do |i|
      puts "Carriage number #{i.number}, kind #{i.kind},
       free space #{i.free_volume}" if i.kind == 'cargo'
      puts "Carriage number #{i.number}, kind #{i.kind},
       free size #{i.free_space}" if i.kind == 'passenger'
    end
  ...
  when command == 'load_carriage'
    ...
    carriage_for_load = train_obj.carriages.each { |i| i if i.number.to_s == carriage_numb }
    carriage_for_load = carriage_for_load[0]
    if train_obj.kind == 'cargo'
      puts 'how much?'
      weight = gets.chomp
      carriage_for_load.loading_goods(weight)
    else
      carriage_for_load.passenger_add
    end
  end
end
